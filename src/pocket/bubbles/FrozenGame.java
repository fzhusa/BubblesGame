...
public class FrozenGame extends GameScreen {
                    ...
                    LevelManager levelManager_arg)
  {
    ...
    //Rect(305, 290, 203 + 240, 265 + 90)
    hurrySprite = new ImageSprite(new Rect(302, 390, 302+32, 390+32),
            hurry_arg);
    ...
    if (FrozenBubble.getMode() == FrozenBubble.GAME_NORMAL) {
      nextBubble = new ImageSprite(new Rect(402, 390, 402 + 32, 390 + 32),  //302, 440, 302 + 32, 440 + 32
                                   bubbles[nextColor]);
    } else {
      nextBubble = new ImageSprite(new Rect(402, 390, 402 + 32, 390 + 32),
  ...
  private void initFrozenify()
  {
    ...
    ImageSprite freezeNextBubble =
        new ImageSprite(new Rect(402, 389, 32, 32), frozenBubbles[nextColor]);
  ...
  private void frozenify()
  {
    ...
    if (frozenifyX < 0) {
      ...
      if (frozenifyY<0) {
        frozenify = false;
        this.addSprite(new ImageSprite(new Rect(170, 150, 170+300, 150+203), gameLost));  //Rect(152, 190, 337, 116)
    ...
    while (bubblePlay[frozenifyX][frozenifyY] == null && frozenifyY >=0) {
      ...
      if (frozenifyX < 0) {
        ...
        if (frozenifyY<0) {
          frozenify = false;
          this.addSprite(new ImageSprite(new Rect(170, 150, 170+300, 150+203),
                                         gameLost));
          soundManager.playSound(FrozenBubble.SOUND_NOH);
  ...
  public boolean play(boolean key_left, boolean key_right, boolean key_fire,
                      double trackball_dx, double touch_dx,double touchx,double touchy,boolean touch_fire)
  {
    int[] move = new int[2];
    ...
    if (endOfGame) {
      ...
      if (move[FIRE] == KEY_UP || hurryTime > 480) {
        if (movingBubble == null && readyToFire) {
          ...
          if(touch_fire)
          {
        //   Log.d("Bubble-touchx", String.valueOf(touchx));
        //   Log.d("Bubble-touchy", String.valueOf(touchy));
           double k=(touchx-310)/(400-touchy);
           
           launchBubblePosition=(Math.atan(k)*180/Math.PI);
           
          
           
        //   Log.d("Bubble-launchBubblePosition", String.valueOf(launchBubblePosition));
           
           launchBubblePosition=20+launchBubblePosition*20/90;
    ...
    if (movingBubble != null) {
      ...
      if (movingBubble.fixed()) {
        if (movingBubble.getSpritePosition().y>=380 &&
          ...
          this.addSprite(new ImageSprite(new Rect(170, 100, 170+300, 100+300),  //Rect(152, 190, 337, 116)
                                         gameWon));
          // TODO
      ...
      if (movingBubble != null) {
        ...
        if (movingBubble.fixed()) {
          if (movingBubble.getSpritePosition().y>=380 &&
            ...
            penguin.updateState(PenguinSprite.STATE_GAME_WON);
            this.addSprite(new ImageSprite(new Rect(170, 100, 170+300, 100+300),
                                           gameWon));
            // TODO
            //highscoreManager.endLevel(nbBubbles);
            endOfGame = true;
